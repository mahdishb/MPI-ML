...
for group_name, group_data in merge_30.groupby(['reaction','metName']):
  ...
  for i in range(0,len(x)):
    for j in range(i+1, len(x)):
      distance = pow((pow(((x[i]-x[j])/eta_max),2) + pow(((y[i]-y[j]))/flx_max,2)),(0.5))
      # print(distance)
      row.append(distance)
  # print(x)
  # print(y)
...
for group_name, group_data in merge_30.groupby(['reaction','metName']):
  ...
  if flx_max == 0:
    flx_max = 1
  x_scaled = [value / eta_max for value in x]
  y_scaled = [value / flx_max for value in y]
  # concat = x + y
  concat = x_scaled + y_scaled
  row = row + concat
  # print(x)
  # print(y)
  features = features.append(pd.DataFrame([row]), ignore_index=True)
